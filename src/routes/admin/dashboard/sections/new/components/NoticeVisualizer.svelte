<script lang="ts">
  ...
  let noticeContents = $state<ParsedText>({
    titulo: '',
    textoNormal: '',
    postdata: ''
  })
  ...
  $effect(() => {
    if(noticeFormState.noticeType === 'Mediano'){
      noticeContents = parseEditorText(noticeContent.raw);
    }
  })
...
<div class="flex-1 flex flex-col m-2 p-4 rounded-xl bg-slate-200">
  ...
  <div class="flex-1 flex items-center justify-center" style="white-space: pre-line;">
    {#if noticeContent.raw == ''}
      Sin vista previa
    {:else}
      título: {noticeContents.titulo}
      texto normal: {noticeContents.textoNormal}
